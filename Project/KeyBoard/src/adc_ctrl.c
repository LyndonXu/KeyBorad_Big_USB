...
void RedressPushRodLimit (void)
{
	...
	ChangeLedState(GET_X(_Led_Caption_EN), GET_Y(_Led_Caption_EN), true);
	...
	while(1)
	{
		...
		{
			if (pKey->u8KeyValue == _Key_Cam_Ctrl_Wide)
			{
				u32MsgSentTime = g_u32SysTickCnt;
				if (pKey->u8KeyState == KEY_DOWN)
				{
					ChangeLedState(GET_X(_Led_Cam_Ctrl_Wide), GET_Y(_Led_Cam_Ctrl_Wide), true);
				}
				else if (pKey->u8KeyState == KEY_UP)
				{
					ChangeLedState(GET_X(_Led_Cam_Ctrl_Wide), GET_Y(_Led_Cam_Ctrl_Wide), false);
		...
		{
			if (pKey->u8KeyValue == _Key_Cam_Ctrl_Tele)
			{
				u32MsgSentTime = g_u32SysTickCnt;
				if (pKey->u8KeyState == KEY_DOWN)
				{
					ChangeLedState(GET_X(_Key_Cam_Ctrl_Tele), GET_Y(_Key_Cam_Ctrl_Tele), true);
				}
				else if (pKey->u8KeyState == KEY_UP)
				{
					ChangeLedState(GET_X(_Key_Cam_Ctrl_Tele), GET_Y(_Key_Cam_Ctrl_Tele), false);
	...
	if (PushRodGetTheRedressLimit(u16UpLimit, u16DownLimit))
	{
		if (WriteSaveData())
		{
			ChangeLedState(GET_X(_Led_Caption_EN), GET_Y(_Led_Caption_EN), false);
			u32MsgSentTime = g_u32SysTickCnt;
...

	{
		...
		{
			boBlink = !boBlink;
			ChangeLedState(GET_X(_Led_Caption_EN), GET_Y(_Led_Caption_EN), boBlink);
			u32MsgSentTime = g_u32SysTickCnt;
